...
- **Chrome Browser Compatibility Fix (August 18, 2025)**: Resolved critical browser compatibility issues where main menu and hamburger menu failed to load on Chrome-based browsers (mobile and desktop) while working properly on Firefox. Root cause: fetchPriority attribute unsupported in older Chrome versions causing React DOM warnings and menu system failures. Fixed by: (1) Removed all fetchPriority attributes from image components and utilities, (2) Implemented dedicated browser-compatibility.js script with Chrome-specific CSS and event handling fixes, (3) Added data-dropdown-trigger attributes for better menu state management, (4) Created critical-path.js for essential mobile viewport and error handling, (5) Fixed CSS aspect-ratio compatibility issues. All browsers now have consistent menu functionality and performance.
- **Complete Resource Localization (August 22, 2025)**: Achieved 100% local resource dependency by downloading and storing all external resources locally. Major accomplishments: (1) Downloaded Google Fonts (Montserrat, Open Sans) as WOFF2 files to /assets/fonts/, (2) Downloaded all Unsplash images to /assets/images/downloaded/ directory, (3) Downloaded FontAwesome CSS and fonts locally with updated paths, (4) Created local-fonts.css replacing Google Fonts CDN, (5) Updated OptimizedImage component with mapUnsplashToLocal utility for automatic local image mapping, (6) Updated performance.css to use local font paths, (7) Removed all preconnect links to external font/image CDNs, (8) Created resource verification script to monitor and block unauthorized external requests. Site now operates with zero external dependencies except for approved third-party services (Brevo, WonderPush). Performance improved with eliminated network latency for fonts and images.
- **Comprehensive Sitemap Generation (August 23, 2025)**: Created complete SEO sitemap system for search engine indexing. Generated XML sitemap with 33 URLs covering all public pages (7 main pages, 15 service pages, 9 industry pages, 2 legal pages), HTML sitemap for user navigation, and robots.txt for search engine crawling instructions. Implemented proper server routes with caching headers, SEO-optimized priority scoring (1.0 for homepage, 0.9 for main categories, 0.8 for core pages), and automated generation script. All sitemap files served with proper MIME types and 24-hour caching. Ready for submission to Google Search Console, Bing Webmaster Tools, and other search engines.